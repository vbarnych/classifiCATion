...
class App extends events.EventEmitter {
  ...
  async getPrediction() {
    //  const path = "C:\\GitLab\\Cats\\1.jpg"
      for (let i = 191; i <= 200; ++i)
      {
        let path = `C:\\GitLab\\Cats\\${i}.jpg`
        const data = await this.neuronNetwork.getInfo(path);
        console.log(data);
        data.id = i;
        const queryResult = this.database.insert('Cats', data)
      }
  }
}
